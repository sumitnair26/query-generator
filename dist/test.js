...
function generateQuery(schemaName, selectColumns, filters = {}) {
    // Find the schema by name
    const schema = schema_1.schemas.find((s) => s.name === schemaName);
    if (!schema) {
    ...
    if (requiredTables.size > 0) {
        relations.forEach((relation) => {
            if (requiredTables.has(relation.table)) {
                ...
                if (type === 'forward') {
                    joinClauses.push(`LEFT JOIN ${table} AS ${alias} ON ${baseTable}.${field} = ${alias}.id`);
                }
                else if (type === 'reverse') {
                    joinClauses.push(`LEFT JOIN ${table} AS ${alias} ON ${alias}.${field} = ${baseTable}.id`);
                }
            }
        });
    }
